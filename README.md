...
- **Phase 7 (in progress):** **`wisdom.cube.device`** — registry + **`DefaultAutomationEngine`**; **`VoiceTurnPipeline`** runs light intents through automation when an engine is supplied (otherwise LLM path). **`Cube.main`** builds one **`InMemoryBehaviourLogStore`** + **`DeviceFixtureStore`** and passes them into **`HttpServerGateway`** so HTTP and voice share state. Use **`wisdom.cube.voice.VoicePipelineFactory.forGateway(...)`** to construct a pipeline against that gateway. **`POST /devices/discover`** (stub); structured device **404**. **Not** included: Home Assistant, real discovery, or a mic loop in **`Cube.main`**.
