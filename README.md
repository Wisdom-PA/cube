...
- **Phase 7 (in progress):** **`wisdom.cube.device`** — registry + **`DefaultAutomationEngine`**. **`docs/F4_INTENT_DIALOGUE_AND_MEMORY.md`** (**F4**): **`VoiceContextChain`** (follow-ups), **`SingleDeviceFallbackIntentClassifier`**, **`DialogueState.EXECUTING`** + **5s listen deadline**, **`SensitiveActionConfirmationPolicy`**, **`MemoryStore.exportProfile`** / **`forgetDay`**. **`VoicePipelineFactory`** wraps the intent classifier and attaches a **`VoiceContextChain`**. **Not:** production LLM runtime, Home Assistant, **`Cube.main`** mic loop.
