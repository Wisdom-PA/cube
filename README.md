...
- **Phase 7 (started):** **`wisdom.cube.device`** — **`LightDeviceRegistry`**, **`InMemoryLightDeviceRegistry`**, **`DefaultAutomationEngine`** (light on/off + brightness by room slug); **`DeviceFixtureStore`** shares the same registry as the HTTP device API. **Not** included: Home Assistant, discovery, or voice pipeline → automation wiring yet.
